...
Human gate: `npx tsx .sandcastle/sdd.ts accept <key>` on `proposing`, `specifying`, and `designing`. The robot does not set `proposed`, `specified`, or `designed`. On `accepting` a person merges the pull request. `sdd:auto-merge` lets the machine rebase it. The machine sets `accepted` only after that merge.
