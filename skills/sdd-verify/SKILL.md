...
2. Run `npx tsx .sandcastle/sdd.ts checks <pr>` and read the failed job logs.
...
7. Infrastructure only: one `sdd:note` thread with the cause, then `npx tsx .sandcastle/sdd.ts wait <issue>`.
